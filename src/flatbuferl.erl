...
%%   ok = flatbuferl:schema_to_module(Schema, mearum_port_protocol).
%%
...
-spec schema_module_content(module(), schema()) -> iolist().
schema_module_content(ModuleName, {Defs, Opts}) ->
    ...
    [
        ...
        "-module(", ModuleStr, ").\n\n",
        "-include(\"flatbuferl_records.hrl\").\n\n",
        "-export([schema/0, encode_envelope/2, decode_envelope/1]).\n\n",
        "%% =============================================================================\n",
        "%% Schema (pre-parsed, compiled into the beam)\n",
        "%% =============================================================================\n\n",
        "-spec schema() -> flatbuferl:schema().\n",
        "schema() ->\n    ",
        erlang_term_to_src({Defs, Opts}, 4),
        ".\n\n",
        "%% =============================================================================\n",
...
%% @private Check if an atom string needs single-quote wrapping.
needs_quote([]) -> true;
needs_quote(Str) ->
    [C | _] = Str,
    (C >= $A andalso C =< $Z) orelse
    not lists:all(fun is_alnum_or_underscore/1, Str).
...
%% @private Convert an Erlang term to valid Erlang source code.
-spec erlang_term_to_src(term(), non_neg_integer()) -> iolist().
erlang_term_to_src(Term, Indent) ->
    erlang_term_to_src(Term, Indent, 0).

-spec erlang_term_to_src(term(), non_neg_integer(), non_neg_integer()) -> iolist().
erlang_term_to_src(Atom, _Indent, _Depth) when is_atom(Atom) ->
    AtomStr = atom_to_list(Atom),
    case erl_scan:reserved_word(Atom) orelse needs_quote(AtomStr) of
        true -> ["'", AtomStr, "'"];
        false -> AtomStr
    end;
erlang_term_to_src(Bin, _Indent, _Depth) when is_binary(Bin) ->
    ["<<\"", binary_to_list(Bin), "\">>"];
erlang_term_to_src(Int, _Indent, _Depth) when is_integer(Int) ->
    integer_to_list(Int);
erlang_term_to_src(Float, _Indent, _Depth) when is_float(Float) ->
    float_to_list(Float);
erlang_term_to_src(List, Indent, Depth) when is_list(List) ->
    ["[",
     lists:join(",\n" ++ lists:duplicate(Indent + Depth * 2, $\s),
                [erlang_term_to_src(E, Indent, Depth + 1) || E <- List]),
     "]"];
erlang_term_to_src(Tuple, Indent, Depth) when is_tuple(Tuple) ->
    Elements = tuple_to_list(Tuple),
    ["{",
     lists:join(",\n" ++ lists:duplicate(Indent + Depth * 2, $\s),
                [erlang_term_to_src(E, Indent, Depth + 1) || E <- Elements]),
     "}"];
erlang_term_to_src(Map, Indent, Depth) when is_map(Map) ->
    ...
    case Pairs of
        ...
        _ ->
            ["#{",
             lists:join(",\n" ++ lists:duplicate(Indent + Depth * 2, $\s),
                        [[erlang_term_to_src(K, Indent, Depth + 1), " => ",
                          erlang_term_to_src(V, Indent, Depth + 1)]
                         || {K, V} <- Pairs]),
             "}"]
    end;
erlang_term_to_src(Other, _Indent, _Depth) ->
